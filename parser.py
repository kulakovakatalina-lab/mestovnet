...
ВАЖНО: artist должен быть извлечён ИСКЛЮЧИТЕЛЬНО из текста поста. НЕ придумывай и НЕ подставляй имена артистов, которых нет в тексте. Если в посте не назван конкретный исполнитель — укажи null.
Каждый объект:
{{
