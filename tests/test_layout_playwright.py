...
# Известный на момент написания тестов долг (ratchet, см. tests/conftest.py):
# на мобильном страницы venues/<slug> горизонтально переполняются из-за
# .genre-hero-* блока, общего с genre.html (не адаптирован под длинные
# название/адрес площадки). Подтверждено на jam-club и rok-n-rolschiki.
# Чинить отдельно — здесь только не блокируем релиз на уже известном баге.
KNOWN_MOBILE_OVERFLOW_PAGES = {"venue"}
