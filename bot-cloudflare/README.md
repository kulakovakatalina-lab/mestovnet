...
### Проверка без карточки в очереди

Администратор может отправить боту команду `/publish_test`. Она запускает
тот же workflow `Publish moderation decision`, что и кнопка модерации, но не
создаёт и не одобряет событие. В GitHub Actions должен появиться успешный
запуск; если в KV нет новых решений, workflow завершится без коммита и сайт
не изменится.
