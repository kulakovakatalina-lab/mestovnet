...
def configured_sources(channels_path: Path) -> dict[str, dict]:
    """Возвращает все включённые источники, в том числе web-афиши."""
    ...
    for group_name in ("channels", "max_channels", "vk_channels", "instagram_channels"):
        for channel in data.get(group_name, []):
            ...
            if label:
                sources[label] = {"title": channel.get("title", label), "kind": group_name}
    # Эти два источника вызываются parser.py без записи в channels.json.
    sources.setdefault("yandex_afisha", {"title": "Яндекс Афиша", "kind": "web"})
    sources.setdefault("afisha_ru", {"title": "Afisha.ru", "kind": "web"})
    return sources
